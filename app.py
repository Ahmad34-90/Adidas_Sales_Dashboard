...
<center><h1 class= "title_test">Adidas Sales Dashboard<h1></center>
'''
with col2:
    st.markdown(html_title, unsafe_allow_html=True)

# -----Adding the KPI matric Rows
colA, colB, colC = st.columns(3)
colA.metric("Total Sales", f"${df['TotalSales'].sum():,.0f}")
colB.metric("Units Sold", f"{df['UnitsSold'].sum():,}")
colC.metric("Total Retailers", df['Retailer'].nunique())
...
with view1:
    expander = st.expander("Retailer Sales")
...
#adding the unit sold as line chart on a secondary y-axis
fig3 =go.Figure()
...
treemap['TotalSales (Formated)'] = treemap['TotalSales'].apply (format_sales)
fig4= px.treemap(treemap,path=['Region','City'], values='TotalSales',
                 hover_name='TotalSales (Formated)',
                 hover_data=['TotalSales (Formated)'],
                 color='City',height=700, width=600)
fig4.update_traces(textinfo='label+value')

with col7:         #----Showing the Treemap chart
